...
Public Class HthEmployeeMaster
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            CON.Open()
            CMD = New SqlCommand("SELECT EpfNo,EmployeeName,Com From HthMaster where com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "'order by Com,EpfNo", CON)
    ...
    Sub MyDataGrid_DeleteCommand(ByVal s As Object, ByVal e As DataGridCommandEventArgs)
        ...
        If (ENTDATE <> Format(CDate(Today()), "MM/dd/yyyy")) Then
            ...
            'TxtMail.Text
            emailMessage.To = "udanip@samsoncorporates.com,health@samsoncorporates.com;palithan@samsoncorporates.com"
            'emailMessage.Attachments.Add("c:\RtnTxt.txt")
            'emailMessage.Cc = str_mail
            'emailMessage.Bcc = em1
            emailMessage.Subject = "Try to Record Delete -Health Fund"
            ...
            emailMessage.Body += "User Name :" + Trim(Trim(L1.Text)) & vbCrLf
        ...
        Else    'Dim MyConn As New SqlConnection(CON)
            Dim DeleteCmd As String = "DELETE from EmpMaster2 Where id =" & I
    ...
    Private Sub TxtEmpNo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtEmpNo.TextChanged
        ...
        ADA = New SqlDataAdapter("SELECT * FROM HthMaster WHERE EpfNo = " & Val(TxtEmpNo.Text) & " and Com='" & Mid(LstCom1.SelectedValue, 1, 3) & "' AND EmpAct='Y'", CON)
        DS = New DataSet
        ...
        If count1 <> 0 Then
            ...
            For Each drt2 In DS.Tables("HthMaster").Rows
                ...
                strEmpIdno = IIf(IsDBNull(drt2("EmpIdno")) = True, "Blank ", drt2("EmpIdno"))
    ...
    Private Sub BttIns_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BttIns.Click
        ...
        If count1 <> 0 Then
            ...
            CMD.Connection = CON
            CMD.CommandText = ("UPDATE HthMaster SET Epfno='" & Trim(TxtEmpNo.Text) & "',Com='" & Mid(LstCom1.SelectedValue, 1, 3) & "',EmployeeName= '" & TxtEmpNam.Text & "',EmpAct= '" & Txtfg.Text & "',EmpIdno= '" & TxtIdNo.Text & "',EmpAdd= '" & TxtEmpAdd.Text & "',OldEpfno= '" & TxtOldepfno.Text & "',OldEpfno1= " & TxtOldepfno1.Text & ",OldEpfno2= " & TxtOldepfno2.Text & ",OldEpfno3= " & TxtOldepfno3.Text & ",OldEpfno4= " & TxtOldepfno4.Text & ",Empcld= '" & Txtchd.Text & "',Dob= '" & Mid(Trim(TxtDoA.Text), 4, 2) + "/" + Mid(Trim(TxtDoA.Text), 1, 2) + "/" + Mid(Trim(TxtDoA.Text), 7, 4) & "' WHERE Epfno=" & TxtEmpNo.Text & " and Com='" & Mid(LstCom1.SelectedValue, 1, 3) & "'")
            CMD.ExecuteNonQuery()

        Else
            'CMD.Connection = CON
            'CMD.CommandText = ("INSERT INTO HthMaster(Epfno,Com,EmployeeName,EmpIdno,EmpAdd,EmpAct,dob,empcld,oldepfno,oldepfno1,oldepfno2,oldepfno3,oldepfno4,Ent_code,Ent_Time,Ent_date) VALUES ('" & TxtEmpNo.Text & "','" & Mid(LstCom1.SelectedValue, 1, 3) & "','" & TxtEmpNam.Text & "','" & TxtIdNo.Text & "','" & TxtEmpAdd.Text & "','" & Txtfg.Text & "','" & Mid(Trim(TxtDoA.Text), 4, 2) + "/" + Mid(Trim(TxtDoA.Text), 1, 2) + "/" + Mid(Trim(TxtDoA.Text), 7, 4) & "','" & Txtchd.Text & "','" & TxtOldepfno.Text & "','" & TxtOldepfno1.Text & "','" & TxtOldepfno2.Text & "','" & TxtOldepfno3.Text & "','" & TxtOldepfno4.Text & "','" & Trim(L1.Text) & "','" & FormatDateTime(TimeOfDay) & "','" & Format(DateValue(Today()), "MM/dd/yyyy") & "')")
            'CMD.ExecuteNonQuery()
        End If
    ...
    Private Sub LstCom1_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LstCom1.SelectedIndexChanged
        ...
        CMD = New SqlCommand("SELECT EpfNo,EmployeeName,Com From HthMaster where com='" & Trim(Mid(Trim(LstCom1.SelectedValue), 1, 3)) & "' order by Com,EpfNo", CON)
