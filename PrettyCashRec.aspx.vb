...
Public Class PrettyCashRec
    ...
    Protected WithEvents LstDate As System.Web.UI.WebControls.DropDownList
    ...
    Protected WithEvents PayAmt1 As System.Web.UI.WebControls.Label
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            ' CMD = New SqlCommand("SELECT  CONVERT(VARCHAR(10),DISTINCT In_RqDate,103) as In_RqDate from Inword where In_com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "' order by In_RqDate ", CON1)
            CMD = New SqlCommand("SELECT  DISTINCT In_RqDate from Inword where In_com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "' order by In_RqDate ", CON1)
            ...
            If Not IsPostBack Then
                While DR.Read
                    ...
                    Try
                        ...
                        TT = Mid(DR.GetValue(0), 4, 2) + "/" + Mid(DR.GetValue(0), 1, 2) + "/" + Mid(DR.GetValue(0), 7, 4)
                    ...
                    LstDate.Items.Add(TT)
                End While
            End If
            DR.Close()


            If Trim(LstDate.SelectedValue) = "" Then
                PayAmt.Text = 0
                PayAmt1.Text = 0

            Else
                CMD = New SqlCommand("SELECT sum(In_PayAmt) as PayAmt from Inword where In_com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "' and In_RqDate='" & Mid(Trim(LstDate.SelectedValue), 4, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 1, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 7, 4) & "' group by In_RqDate ", CON1)
                SS = "SELECT sum(In_PayAmt) as PayAmt from Inword where In_com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "' and In_RqDate='" & Mid(Trim(LstDate.SelectedValue), 4, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 1, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 7, 4) & "' group by In_RqDate "
                DR = CMD.ExecuteReader
                While DR.Read
                    TT = DR.GetValue(0)
                    PayAmt.Text = TT
                    PayAmt1.Text = TT
                    PayAmt.Text = "      " + Format(DR.GetValue(0), "###,###,###.00")
                End While
                DR.Close()
    ...
    Public Sub BindTheGrid2()
        ...
        ' Dim cmdCustomers As SqlCommand = New SqlCommand("Select a.id as id,a.Out_Vno as Out_Vno ,b.Emp_no as Emp_no , b.Emp_Nam as Emp_Nam , b.Emp_Dep as Emp_Dep, b.Emp_Com as Emp_Com,a.Out_Amt as Out_Amt from OutWord a,Employee b where (a.Out_Eno=b.Emp_no) and a.Out_Date='" & Mid(Trim(TxtInvDt.Text), 4, 2) + "/" + Mid(Trim(TxtInvDt.Text), 1, 2) + "/" + Mid(Trim(TxtInvDt.Text), 7, 4) & "'", CON1)
        Dim cmdCustomers As SqlCommand = New SqlCommand("Select  id,Out_Vno ,Out_Eno as Emp_no, REPLACE(CONVERT(VARCHAR,CONVERT(MONEY,Out_Amt),1), '','') as Out_Amt, Convert(Varchar(10),Out_Date,103) as Out_Date from OutWord where Out_Flg='Y' and Typ='1'", CON1)
        cmdCustomers.CommandType = CommandType.Text
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        CMD.Connection = CON1
        CMD.CommandText = ("INSERT INTO AddCash(Cash_dt,Cash_Time,Ref,Q1,Q2,Q3,Q4,Q5,Q6,Q7,Q8,Q9,Q10,Q11,Amt,Typ,Com) VALUES ('" & Mid(Trim(TxtInvDt.Text), 4, 2) + "/" + Mid(Trim(TxtInvDt.Text), 1, 2) + "/" + Mid(Trim(TxtInvDt.Text), 7, 4) & "','" & SS & "','" & TxtDes.Text & "','" & Trim(TxtQ1.Text) & "','" & Trim(TxtQ2.Text) & "','" & Trim(TxtQ3.Text) & "','" & Trim(TxtQ4.Text) & "','" & Trim(TxtQ5.Text) & "','" & Trim(TxtQ6.Text) & "','" & Trim(TxtQ7.Text) & "','" & Trim(TxtQ8.Text) & "','" & Trim(TxtQ9.Text) & "','" & Trim(TxtQ10.Text) & "','" & Trim(TxtQ11.Text) & "','" & Trim(TxtAmt1.Text) & "','1','" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "')")
        ...
        CMD.Connection = CON1
        CMD.CommandText = ("UPDATE Imprest SET Imp_Pay=Imp_Pay-" & Val(PayAmt1.Text) & " where Imp_Com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "'")
        CMD.ExecuteNonQuery()

        CMD.Connection = CON1
        CMD.CommandText = ("UPDATE SignCashEnd SET Pay_Amt= Pay_Amt+" & Val(PayAmt.Text) & " where Com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "' and typ='1' and Vou_Date='" & Mid(Trim(TxtInvDt.Text), 4, 2) + "/" + Mid(Trim(TxtInvDt.Text), 1, 2) + "/" + Mid(Trim(TxtInvDt.Text), 7, 4) & "'")
        CMD.ExecuteNonQuery()
        ...
        SS = "UPDATE Inword SET In_Flg='N' where In_Com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "' and In_RqDate='" & Mid(Trim(LstDate.SelectedValue), 4, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 4, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 7, 4) & "'"
        CMD.Connection = CON1
        CMD.CommandText = ("UPDATE Inword SET In_Flg='N',In_FdDate='" & Mid(Trim(TxtInvDt.Text), 4, 2) + "/" + Mid(Trim(TxtInvDt.Text), 1, 2) + "/" + Mid(Trim(TxtInvDt.Text), 7, 4) & "' where In_Com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "' and In_RqDate='" & Mid(Trim(LstDate.SelectedValue), 4, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 1, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 7, 4) & "'")
        CMD.ExecuteNonQuery()



        CMD = New SqlCommand("SELECT  DISTINCT In_RqDate from Inword where In_com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "' and (In_flg='Y' and In_flg1='N') order by In_RqDate ", CON1)
        ...
        While DR.Read
            ...
            TT = Format(DR.GetValue(0), "dd/MM/yyyy")
            LstDate.Items.Add(TT)
        End While
        ' End If
        DR.Close()
    ...
    Private Sub TxtQ1_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtQ1.TextChanged
        ...
        If Val(TxtAmt1.Text) = Val(PayAmt1.Text) Then
            BtnSave.Enabled = True
        End If
    End Sub
    ...
    Private Sub LstCom1_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LstCom1.SelectedIndexChanged
        ...
        Else
            ...
            CMD = New SqlCommand("SELECT sum(In_PayAmt) as PayAmt from Inword where (In_flg='Y' and In_flg1='N') and In_com='" & Mid(Trim(LstCom1.SelectedValue), 1, 3) & "' and In_RqDate='" & Mid(Trim(LstDate.SelectedValue), 4, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 1, 2) + "/" + Mid(Trim(LstDate.SelectedValue), 7, 4) & "' group by In_RqDate ", CON1)
