...
#End Region
    ...
    Private Sub Data4()
        ...
        Dim cmdCustomers As SqlCommand = New SqlCommand("Select EmpMaster.Emp_No as Raw_ItmCode,EmpMaster.Emp_No as Raw_SOH,EmpMaster.Emp_No as Raw_des ,EmpMaster.Emp_Nam as Itm_pri,EmpMaster.OldEpfNo as Raw_opStk,EmpMaster.Emp_No as  Raw_PhStock,EmpMaster.Emp_No as Raw_rec from EmpMaster where EmpMaster.Emp_com='" & Trim(Mid(LstCat.SelectedValue, 1, 3)) & "' AND EmpMaster.EmpAct='Y' and EmpMaster.EmpDel='N' ORDER BY STR(EmpMaster.Emp_No)", CON)
    ...
    Private Sub ButSub_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButSub.Click
        ...
        For Each gridrow In DataGrid1.Items
            ...
            If Trim(t1) <> Trim(t6) Then
                ...
                '''FormatDateTime(TimeOfDay) & "','" & Format(DateValue(Today()), "MM/dd/yyyy") &
                SS = "UPDATE HthMaster SET EmpAct='" & t6 & "',Act_Code='" & Trim(L1.Text) & "',Act_time='" & FormatDateTime(TimeOfDay) & "',Act_Date='" & Format(DateValue(Today()), "MM/dd/yyyy") & "',Act_Act='" & Trim(t6) & "' where EpfNo='" & Trim(t2) & "' and Com='" & Trim(Mid(LstCat.SelectedValue, 1, 3)) & "'"
                CMD.CommandText = ("UPDATE EmpMaster SET Emp_No='" & Trim(t6) & "',OldEpfno='" & Trim(t1) & "', Ent_Code='" & Trim(L1.Text) & "',Ent_time='" & FormatDateTime(TimeOfDay) & "',Ent_Date='" & Format(DateValue(Today()), "MM/dd/yyyy") & "' where Emp_No='" & Trim(t1) & "' and Emp_Com='" & Trim(Mid(LstCat.SelectedValue, 1, 3)) & "'")
                CMD.ExecuteNonQuery()
                ...
                CMD.CommandText = ("UPDATE eMPMaster1 SET Emp_No='" & Trim(t6) & "', Ent_Code='" & Trim(L1.Text) & "',Ent_time='" & FormatDateTime(TimeOfDay) & "',Ent_Date='" & Format(DateValue(Today()), "MM/dd/yyyy") & "' where Emp_No='" & Trim(t1) & "' and Emp_Com='" & Trim(Mid(LstCat.SelectedValue, 1, 3)) & "'")
                CMD.ExecuteNonQuery()
