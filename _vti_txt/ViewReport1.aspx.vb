...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else
            CMD = New SqlCommand("SELECT Form_name,Form_level FROM FormTable where Form_name='ViewReport1.aspx' and Form_level='" & Global.PwLevel & "'", CON)
    ...
    Public Sub BindTheGrid()
        ...
        fdate1 = Mid(TxtFdt.Text, 4, 2) + "/" + Mid(TxtFdt.Text, 1, 2) + "/" + Mid(TxtFdt.Text, 7, 4)

        fdate2 = Mid(Txttdt.Text, 4, 2) + "/" + Mid(Txttdt.Text, 1, 2) + "/" + Mid(Txttdt.Text, 7, 4)
        ...
        ss = "Select InhInv.Inv_No,InhInv.Inv_Com, Format(InhInv.Inv_Dt,'dd/MM/yyyy') as InvDt,InhInv.Inv_pono, InhInv.Inv_TotVal, InhInv.Inv_TotQty,InhInv.Inv_flag2 from (InhInv) where (((InhInv.Inv_dt)>=#" & fdate1 & "# And (InhInv.Inv_dt)<=#" & fdate2 & "#) and ((InhInv.Inv_Flag2)='" & ss1 & "'))"
        objConn = New SqlConnection
        objConn.ConnectionString = ("data source=(local);initial catalog=Insu; user id=sa;password=tstc123")
        objConn.Open()
        Dim cmdCustomers As SqlCommand = New SqlCommand("Select InhInv.Inv_No,InhInv.Inv_Com, Convert(varchar(10),InhInv.Inv_Dt,103) as InvDt,InhInv.Inv_pono, InhInv.Inv_TotVal as InvTotval, InhInv.Inv_TotQty as InvTotQty,InhInv.Inv_flag2 from InhInv where InhInv.Inv_dt>='" & fdate1 & "' And InhInv.Inv_dt<='" & fdate2 & "' and InhInv.Inv_Flag2='" & ss1 & "'", objConn)
