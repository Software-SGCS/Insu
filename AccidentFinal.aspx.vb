...
Public Class AccidentFinal
    ...
    Dim Count As Double
    Dim body As String
    Dim fg1, fg2, fg3, fg4, fg5 As String
    ...
    Dim Teamt, Tamt1, Tamt2 As Double
    ...
    Protected WithEvents Svgfg As System.Web.UI.WebControls.DropDownList
    Protected WithEvents Flfg As System.Web.UI.WebControls.DropDownList
    Protected WithEvents Label32 As System.Web.UI.WebControls.Label
    Protected WithEvents Cfg As System.Web.UI.WebControls.DropDownList
    Protected WithEvents TxtFinalAmt As System.Web.UI.WebControls.TextBox
    Protected WithEvents TxtChqNo As System.Web.UI.WebControls.TextBox
    Protected WithEvents Txtpfg As System.Web.UI.WebControls.TextBox
    Protected WithEvents Txtdfg As System.Web.UI.WebControls.TextBox
    ...
    Protected WithEvents TxtSdt As System.Web.UI.WebControls.TextBox
    Protected WithEvents Label36 As System.Web.UI.WebControls.Label
    Protected WithEvents TxtToTPre As System.Web.UI.WebControls.TextBox
    ...
    Protected WithEvents TxtCstAmt As System.Web.UI.WebControls.TextBox
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        CMD = New SqlCommand("SELECT Id,Flag From PoliceFlag where id<=2 order by Flag", CON)
        DR = CMD.ExecuteReader
        If Not IsPostBack Then
            While DR.Read
                TT = DR.GetValue(1)
    ...
    Private Sub LstId_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LstId.SelectedIndexChanged
        ...
        If Count <> 0 Then
            ...
            For Each dr1 In DS.Tables("VEHMAST").Rows
                ...
                If IsDBNull(dr1("Veh_Sdt")) Then
                    Tamt1 = 0

                Else
                    fg1 = Format(dr1("Veh_Sdt"), "dd/MM/yyyy")
                    ...
                    Tamt1 = Val(Mid(fg1, 7, 4))
                    Tamt2 = Val(Mid(Format(Today, "dd/MM/yyyy"), 7, 4))
                ...
                End If
                ' Format((val1), "###,###,###.##")
                TxtToTPre.Text = Right(Space(20) + Format((Val(dr1("Veh_TotChqPay")) * Tamt1), "###,###,###.00"), 20)
    ...
    Private Sub BtnApp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnApp.Click
        ...
        s3 = Mid(Trim(TxtPaydt.Text), 4, 2) + "/" + Mid(Trim(TxtPaydt.Text), 1, 2) + "/" + Mid(Trim(TxtPaydt.Text), 7, 4)
        ...
        Dim svgfg1, flfg1, cfg1 As String
        If Trim(Flfg.SelectedValue) = "Yes" Then
            flfg1 = "Y"
        Else
            flfg1 = "N"
        End If

        If Trim(Svgfg.SelectedValue) = "Yes" Then
            svgfg1 = "Y"
        Else
            If Trim(Svgfg.SelectedValue) = "No" Then
                svgfg1 = "N"
            Else
                svgfg1 = "X"
            End If
        End If
        If Trim(Cfg.SelectedValue) = "Yes" Then
            cfg1 = "Y"
        Else
            cfg1 = "N"
        End If
        ...
        ' If BtnAdd.Enabled = False Then
        CMD.CommandText = ("UPDATE AccPayNew SET Company='" & com & "',Chq_no='" & Trim(TxtChqNo.Text) & "',Final_Amt='" & Trim(TxtFinalAmt.Text) & "',Cost_Amt='" & Trim(TxtCstAmt.Text) & "',Pay_Date='" & s3 & "',Final_fg='" & cfg1 & "',Final_Img='" & flfg1 & "',Remakes='" & Trim(txtRem.Text) & "',Fin_svg='" & Trim(svgfg1) & "' WHERE Veh_no='" & Trim(LstId.SelectedValue) & "' and Claim_No='" & Trim(TxtVehClNo.Text) & "'")
        CMD.ExecuteNonQuery()
        ' Else
        'CMD.CommandText = ("INSERT INTO AccPayNew (Company,Acc_Date,Ext_Amt1,Ext_Amt2,App_Amt1,App_Amt2,Claim_No,Final_fg,Dmg_fg,Plc_fg,Remakes,Veh_No) VALUES ('" & com & "','" & s1 & "','" & TxtEstval1.Text & "','" & TxtEstval2.Text & "','" & TxtAppVal1.Text & "','" & TxtAppVal2.Text & "','" & Trim(TxtVehClNo.Text) & "','" & finfg1 & "','" & dmgfg1 & "','" & Trim(plcfg1) & "','" & Trim(txtRem.Text) & "','" & LstId.SelectedValue & "')")
        'CMD.ExecuteNonQuery()
    ...
    Private Sub TxtVehClNo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtVehClNo.TextChanged
        ...
        If Count <> 0 Then
            For Each Dtr1 In DS.Tables("AccPayNew").Rows
                ...
                fg3 = IIf((IsDBNull(Dtr1("Fin_svg"))), " ", Dtr1("Fin_svg"))

                fg4 = IIf((IsDBNull(Dtr1("Final_img"))), " ", Dtr1("Final_img"))


                fg5 = IIf((IsDBNull(Dtr1("Final_fg"))), " ", Dtr1("Final_fg"))

                Txtpfg.Text = Trim(fg1)

                Txtdfg.Text = Trim(fg2)
                If fg3 = "Y" Then
                    Svgfg.SelectedItem.Text = "Yes"
                    Svgfg.Items.Add("No")
                    Svgfg.Items.Add("Not Available")
                Else
                    If fg3 = "N" Then
                        Svgfg.SelectedItem.Text = "No"
                        Svgfg.Items.Add("Yes")
                        Svgfg.Items.Add("Not Available")


                    Else
                        Svgfg.SelectedItem.Text = "Not Available"
                        Svgfg.Items.Add("Yes")
                        Svgfg.Items.Add("No")
                ...
                If fg4 = "Y" Then
                    Flfg.SelectedItem.Text = "Yes"
                    Flfg.Items.Add("No")
                Else
                    Flfg.SelectedItem.Text = "No"
                    Flfg.Items.Add("Yes")
                End If

                If fg5 = "N" Then
                    Cfg.SelectedItem.Text = "No"
                    TxtFinalAmt.ReadOnly = False
                    TxtChqNo.ReadOnly = False
                    BtnApp.Enabled = True

                Else
                    Cfg.SelectedItem.Text = "Yes"
                    Cfg.Items.Add("No")
                    TxtFinalAmt.ReadOnly = True
                ...
                TxtChqNo.Text = IIf((IsDBNull(Dtr1("Chq_No"))), " ", Dtr1("Chq_No"))
                TxtFinalAmt.Text = IIf((IsDBNull(Dtr1("Final_Amt"))), " ", Dtr1("Final_Amt"))
    ...
    Public Sub BindTheGrid()
        ...
        Dim cmdCustomers As SqlCommand = New SqlCommand("Select Id,convert(varchar(10),Acc_Date,103) as Acc_Date,convert(decimal(11,2),Final_Amt,2) as Final_Amt ,convert(decimal(11,2),Ext_Amt1,2) as Ext_Amt1 ,convert(decimal(11,2),Ext_Amt2,2) as Ext_Amt2,convert(decimal(11,2),App_Amt1,2) as App_Amt1,convert(decimal(11,2),App_Amt2,2) as App_Amt2,Claim_No,Chq_No,RepPer from AccPayNew where Veh_no='" & Trim(LstId.SelectedItem.Text) & "' and Final_fg='Y'", CON)
    ...
    Public Sub BindTheGrid1()
        ...
        CON.ConnectionString = ("data source=(local);initial catalog=Insu; user id=sa;password=tstc123")
        CON.Open()
        Dim cmdCustomers As SqlCommand = New SqlCommand("Select Id,convert(varchar(10),Acc_Date,103) as Acc_Date,convert(decimal(11,2),Ext_Amt1,2) as Ext_Amt1,convert(decimal(11,2),Final_Amt,2) as Final_Amt,convert(decimal(11,2),Ext_Amt2,2) as Ext_Amt2,convert(decimal(11,2),App_Amt1,2) as App_Amt1,convert(decimal(11,2),App_Amt2,2) as App_Amt2,Claim_No,Dmg_fg,Plc_fg,Remakes,Fin_Svg,Final_Img,RepPer from AccPayNew where Veh_no='" & Trim(LstId.SelectedItem.Text) & "' and Final_fg='N'", CON)
    ...
    Private Sub Calendar2_SelectionChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Calendar2.SelectionChanged
        ...
        BtnApp.Visible = True
        'PlcFg.Visible = True
        TxtPaydt.Text = Format((Calendar2.SelectedDate).ToString("dd/MM/yyyy"))
        ' s3 = Mid(Trim(TxtAcidt.Text), 4, 2) + "/" + Mid(Trim(TxtAcidt.Text), 1, 2) + "/" + Mid(Trim(TxtAcidt.Text), 7, 4)
        Calendar2.Visible = False
    End Sub

    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Dim DSS As VehAcCReport1 = New VehAcCReport1
