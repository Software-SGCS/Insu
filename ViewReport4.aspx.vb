...
Public Class ViewReport4
    ...
    Dim Finvno, Tinvno, Invno1, InvCom1, Invdt1, InvFlag11, InvPono1, InvItmNo1, InvQty1, InvVal1, InvPri1, Invvat1, Invdis1, Invflag31, invper1 As String
    ...
    Protected WithEvents LstFInvNo As System.Web.UI.WebControls.DropDownList
    Protected WithEvents LstTInvNo As System.Web.UI.WebControls.DropDownList
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else
            CMD = New SqlCommand("SELECT Form_name,Form_level FROM FormTable where Form_name='ViewReport4.aspx' and Form_level='" & Global.PwLevel & "'", CON)
        ...
        If Not IsPostBack Then
            ...
            'BindTheGrid()
            CMD = New SqlCommand("SELECT Veh_CertifyNo From VehMast where Allow1='Approved' order by Veh_CertifyNo", CON)
            ...
            If Not IsPostBack Then
                While DR.Read
                    ...
                    LstFInvNo.Items.Add(DR.GetValue(0))
                    LstTInvNo.Items.Add(DR.GetValue(0))
    ...
    Public Sub BindTheGrid()
        ...
        Finvno = Trim(LstFInvNo.SelectedValue)
        Tinvno = Trim(LstTInvNo.SelectedValue)
        ...
        ss = "Select InhInv.Inv_No,InhInv.Inv_Com, Format(InhInv.Inv_Dt,'dd/MM/yyyy') as InvDt,InhInv.Inv_pono, InhInv.Inv_TotVal, InhInv.Inv_TotQty,InhInv.Inv_flag2 from (InhInv) where where (((InhInv.Inv_No)>='" & Finvno & "' And (InhInv.Inv_No)<='" & Tinvno & "') and ((InhInv.Inv_Flag2)='" & ss1 & "'))"
        objConn = New SqlConnection
        objConn.ConnectionString = ("data source=(local);initial catalog=Insu; user id=sa;password=tstc123")
        objConn.Open()
        Dim cmdCustomers As SqlCommand = New SqlCommand("Select Veh_No,Veh_Com, Veh_Typ,convert(varchar(10),Veh_EfDt,103) as Veh_EfDt, convert(varchar(10),Veh_Exdt,103) as Veh_Exdt, Veh_tpay  from VehMast where (((VehMast.Veh_CertifyNo)>='" & Finvno & "' And (VehMast.Veh_CertifyNo)<='" & Tinvno & "') and ((VehMast.Allow1)='" & ss1 & "'))", objConn)
    ...
    Private Sub LstFInvNo_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LstFInvNo.SelectedIndexChanged
        Finvno = Trim(LstFInvNo.SelectedValue)
        Tinvno = Trim(LstTInvNo.SelectedValue)
        ...
        ss1 = "Approved"
        'Dim cmdCustomers As SqlCommand = New SqlCommand("Select id,Inv_Itmno,Inv_Des, format(Inv_pr,'#,###.00') as InvPr,format(Inv_Qty,'#,###') as InvQty, format(Inv_val,'#,###.00') as InvVal from InlTemp", CON)
        ss = "Select InhInv.Inv_No,InhInv.Inv_Com, Format(InhInv.Inv_Dt,'dd/MM/yyyy') as InvDt,InhInv.Inv_pono, InhInv.Inv_TotVal, InhInv.Inv_TotQty,InhInv.Inv_flag2 from (InhInv) where (((InhInv.Inv_No)>='" & Finvno & "' And (InhInv.Inv_No)<='" & Tinvno & "') and ((InhInv.Inv_Flag2)='" & ss1 & "'))"
        objConn = New SqlConnection
        objConn.ConnectionString = ("data source=(local);initial catalog=Insu; user id=sa;password=tstc123")
        objConn.Open()
        ' Dim cmdCustomers As SqlCommand = New SqlCommand("Select Veh_No,Veh_Com, Veh_Typ,Format(Veh_EfDt,'dd/MM/yyyy') as Veh_EfDt, Format(Veh_Exdt,'dd/MM/yyyy') as Veh_Exdt, Veh_tpay ,Veh_val,ALLOW2,Veh_bpay,Veh_rcc,Veh_tr from VehMast where (((VehMast.Inv_No)>='" & Finvno & "' And (VehMast.Inv_No)<='" & Tinvno & "') and ((VehMast.Allow1)='" & ss1 & "'))", objConn)
        ' Dim cmdCustomers As SqlCommand = New SqlCommand("Select InhInv.Inv_No,InhInv.Inv_Com, InhInv.Inv_Dt,InhInv.Inv_pono, InhInv.Inv_TotVal, InhInv.Inv_TotQty,InhInv.Inv_flag2 from (InhInv) where (((InhInv.Inv_dt)>=#6/1/2006# And (InhInv.Inv_dt)<=#6/30/2006#) and ((InhInv.Inv_Flag2)='" & ss1 & "'))", objConn)
        'Dim cmdCustomers As SqlCommand = New SqlCommand("Select InhInv.Inv_No,InhInv.Inv_Com, InhInv.Inv_Dt,InhInv.Inv_pono, InhInv.Inv_TotVal, InhInv.Inv_TotQty,InhInv.Inv_flag2 from (InhInv) where InhInv.Inv_Flag2='" & ss1 & "'", objConn)
        Dim cmdCustomers As SqlCommand = New SqlCommand("Select Veh_No,Veh_Com, Veh_Typ,convert(varchar(10),Veh_EfDt,103) as Veh_EfDt, convert(varchar(10),Veh_Exdt,103) as Veh_Exdt, Veh_tpay  from VehMast where (((VehMast.Veh_CertifyNo)>='" & Finvno & "' And (VehMast.Veh_CertifyNo)<='" & Tinvno & "') and ((VehMast.Allow1)='" & ss1 & "'))", objConn)
        cmdCustomers.CommandType = CommandType.Text
    ...
    Private Sub LstTInvNo_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LstTInvNo.SelectedIndexChanged
        Finvno = Trim(LstFInvNo.SelectedValue)
        Tinvno = Trim(LstTInvNo.SelectedValue)
